...
import { 
  ...
  Plus,
  Trash2,
  ChevronUp, Linkedin, Instagram, Facebook
} from "lucide-react";
...
export default function BrandsPage() {
  ...
  useEffect(() => {
    syncCart(); // Load immediately on mount
  ...
  const filteredProducts = products.filter(p => {
    const matchesFilter = activeFilter === "All" || p.brands?.includes(activeFilter);
     ...
     {/* --- 1. NAVIGATION (FROSTED GLASS / MALIWANAG STYLE) --- */}
      ...
      <section className="relative h-[50vh] w-full flex overflow-hidden bg-black">
        ...
        <div className="absolute bottom-12 left-12 z-10">
            <h1 className="text-white text-5xl md:text-7xl font-black uppercase tracking-tighter opacity-20">DISRUPTIVE <br/> PRODUCTS</h1>
        </div>
      </section>
                ...
                {["All", "LIT", "ZUMTOBEL"].map((tab) => (
                    <button
                        ...
                        className={`px-8 py-2 rounded-full text-[9px] font-black uppercase tracking-widest transition-all ${
                            activeFilter === tab ? "bg-[#d11a2a] text-white shadow-lg" : "bg-gray-100 text-gray-400 hover:bg-gray-200"
                        }`}
                    >
                        {tab}
                    </button>
          ...
          <>
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} className="fixed top-0 right-0 h-full w-full max-w-sm bg-white z-[2001] shadow-2xl flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {quoteCart.length === 0 ? (
                  <p className="text-center text-gray-400 text-[10px] font-bold uppercase py-10">Cart is empty</p>
                ) : (
                  quoteCart.map((item) => (
                    <div key={item.id} className="flex gap-4 p-3 bg-gray-50 rounded-2xl relative group hover:bg-white border border-transparent hover:border-gray-100 transition-all">
                      <div className="w-16 h-16 bg-white rounded-lg flex items-center justify-center p-2">
                        <img src={item.mainImage} className="w-full h-full object-contain" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="text-[10px] font-black uppercase leading-tight truncate">{item.name}</h4>
                        <p className="text-[9px] text-gray-400 uppercase mt-1">SKU: {item.sku}</p>
                      </div>
                      <button onClick={() => removeFromQuote(item.id)} className="text-gray-300 hover:text-red-500 transition-colors p-1"><Trash2 size={16}/></button>
                    </div>
                  ))
                )}
              </div>
              <div className="p-6 border-t bg-white">
                <Link href="/quote-request-form" className={`block w-full py-5 text-center rounded-2xl font-black uppercase tracking-widest text-[11px] transition-all ${quoteCart.length > 0 ? "bg-[#d11a2a] text-white shadow-lg shadow-red-500/20" : "bg-gray-100 text-gray-400 pointer-events-none"}`}>
                  Proceed to Inquiry
                </Link>
              </div>
            </motion.div>
