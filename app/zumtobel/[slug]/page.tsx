...
useEffect(() => {
  const fetchData = async () => {
    ...
    try {
      ...
      // 1. HANAPIN ANG PRODUCT (Slug First, then ID Fallback)
      // Sinusubukan muna nating hanapin ang document gamit ang 'slug' field
      const qSlug = query(
      ...
      if (!slugSnap.empty) {
        ...
        // Fallback: Hanapin gamit ang Document ID
      ...
      // 2. KUNG MAY NAHANAP NA PRODUCT, FETCH REVIEWS & LOG ACTIVITY
      if (foundProduct) {
        setProduct(foundProduct);
        setActiveImage(foundProduct.mainImage || "");

        // Mag-log ng activity gamit ang name at sku
        ...
        const filtered = relatedSnap.docs
          ...
          .slice(0, 12); // Siguradong 12 items lang
  ...
  fetchData();
}, [params?.slug]); // Mag-re-run kapag nagbago ang URL slug
...
<div className="relative h-[450px] md:h-[600px] w-full overflow-hidden bg-[#050505]">
  {/* SOLID BLACK BACKGROUND */}
  <div className="absolute inset-0 bg-[#050505]">
    {/* Subtle texture para premium tignan */}
    <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,_var(--tw-gradient-stops))] from-white/[0.03] via-transparent to-transparent opacity-70" />
  </div>
  ...
  <div className="absolute bottom-0 left-0 w-full p-8 md:p-20 z-10">
    <div className="max-w-7xl mx-auto">
      <div className="space-y-6">
        ...
        {/* GREY BRAND LABEL */}
        <div className="inline-flex items-center gap-3 bg-slate-800/40 backdrop-blur-xl border border-slate-700 px-4 py-2 rounded-sm shadow-2xl">
          {/* Silver/Grey pulse */}
          <div className="w-2 h-2 rounded-full bg-slate-400 animate-pulse shadow-[0_0_8px_rgba(148,163,184,0.5)]" />
          <span className="text-slate-100 text-[11px] font-black uppercase tracking-[0.5em]">
            {product?.brands?.[0] || "Zumtobel"} 
          </span>
        </div>

        {/* PRODUCT NAME (White on Black) */}
        <h1 className="text-white text-5xl md:text-9xl font-black uppercase tracking-tighter leading-[0.8] drop-shadow-2xl italic">
          {product?.name || "Premium Series"}
        </h1>

        {/* DESCRIPTION WITH GREY ACCENT */}
        <div className="max-w-2xl border-l-4 border-slate-600 pl-8 py-2">
          <p className="text-slate-400 text-sm md:text-xl font-light leading-relaxed tracking-wide">
            {product?.shortDescription || "Innovative lighting solutions that combine aesthetic grace with functional excellence for architectural mastery."}
          </p>
        </div>
        ...
        <main className="max-w-6xl mx-auto px-4 md:px-6 mt-8 md:mt-12">
          ...
          <nav className="flex items-center gap-2 mb-6 text-[9px] font-black uppercase tracking-widest text-gray-400">
            ...
            <Link href="/zumtobel" className="hover:text-black transition-colors">Products</Link>
            <ChevronRight size={10} />
