...
export default function QuoteCartPanel() {
  ...
  // --- BAGONG FUNCTION PARA SA QUANTITY ---
  ...
  return (
    <>
      {/* FLOATING BUTTON */}
      {!isCartOpen && (
        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="fixed bottom-10 right-10 z-[100]">
          <button onClick={() => setIsCartOpen(true)} className="group relative flex items-center gap-4 bg-black text-white pl-6 pr-2 py-2 rounded-full shadow-2xl hover:scale-105 transition-all border border-white/10">
      ...
      <AnimatePresence>
        {isCartOpen && (
          <>
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25 }} className="fixed top-0 right-0 h-full w-full max-w-sm bg-white z-[2001] shadow-2xl flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {quoteCart.map((item) => (
                  <div key={item.id} className="flex gap-4 p-4 bg-gray-50 rounded-[20px] relative border border-transparent hover:border-gray-100 transition-all">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3">
                        <button 
                          ...
                          className="p-1 bg-white border border-gray-200 rounded-md hover:text-[#d11a2a] transition-colors"
                        >
                          <Minus size={12} />
                        </button>
                    ...
                    <button onClick={() => removeFromQuote(item.id)} className="text-gray-300 hover:text-red-500 transition-colors self-start"><Trash2 size={16}/></button>
                  </div>
                ))}
              </div>

              <div className="p-6 border-t">
                <Link href="/checkout" className="block w-full py-5 bg-[#d11a2a] text-white text-center rounded-2xl font-black uppercase text-[11px] tracking-widest shadow-lg shadow-red-500/20 active:scale-95 transition-transform">
                  Proceed to Inquiry
                </Link>
              </div>
            </motion.div>
